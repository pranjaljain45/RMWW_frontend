...
const TiesBowTies = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filtered = res.data.filter(item => item.subcategory === "Tie Bowtie");
        setTieData(filtered);
      } catch (error) {
        console.error("Error fetching ties & bow ties data", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const firstItem = tieData[0];

  return (
    <div className="ties-page">
      {firstItem && (
