  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filtered = res.data.filter(item => item.subcategory === "sareesBlouses");
        setSareeData(filtered);
      } catch (error) {
        console.error("Error fetching Sarees & Blouse data", error);
      } finally {
        setLoading(false);
      }
    };
