  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filtered = res.data.filter(item => item.subcategory === "banglesBracelets");
        setBangleData(filtered);
      } catch (error) {
        console.error("Error fetching banglesBracelets data", error);
      } finally {
        setLoading(false);
      }
    };
...
export default BanglesBracelets;
