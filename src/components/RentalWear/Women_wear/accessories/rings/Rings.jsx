  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filtered = res.data.filter(item => item.subcategory === "rings");
        setRingData(filtered);
      } catch (error) {
        console.error("Error fetching rings data", error);
      } finally {
        setLoading(false);
      }
    };
