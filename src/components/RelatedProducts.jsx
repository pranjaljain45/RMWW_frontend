...
const RelatedProducts = ({ gender, category, subcategory }) => {
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 gap-y-6">
                {related.length > 0 &&
                    related.map((item) => {
                        ...
                        return (
                            <div key={itemId} className="p-3 hover:shadow-md transition cursor-pointer">

                                <Link
                                    to={`/${item.gender}/${item.category}/${item.subcategory}/${itemId}`}
                                    state={{ product: item }}
                                >
                                    <img
                                        src={item.imageUrl}
